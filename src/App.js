...
function App() {
  ...
  const [upDownNasdaq, setUpDownNasdaq] = useState("0");
  const [upDownCorn, setUpDownCorn] = useState("0");
  const [upDownGasoline, setUpDownGasoline] = useState("0");
  ...
  useEffect(() => {
    async function getData() {
      ...
      setUpDownNasdaq(result1.up_down_nasdaq);
      setUpDownCorn(result1.up_down_corn);
      setUpDownGasoline(result1.up_down_gasoline);
    }
    getData();
  }, []);
  ...
  return (
    <div className="wrapper">
      <header>
        ...
        <p>
          Move the slider or type in a year range to check the stock market!
        </p>
      </header>
      ...
      <div className="stockData">
        ...
        <div className="stocksTable">
          <EstimationsTable
            estimated_ndaq={estimatedNasdaq}
            estimated_corn={estimatedCorn}
            estimated_gasoline={estimatedGasoline}
            upDownNasdaq={upDownNasdaq}
            upDownCorn={upDownCorn}
            upDownGasoline={upDownGasoline}
          />
        </div>
      </div>
    </div>
